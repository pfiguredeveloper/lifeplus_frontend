...
<div class="main-layout">
    <div id="exTab2">
        <ul class="nav nav-tabs">
            <!-- <li >
                <a href="#10" data-toggle="tab"><b>1. Client Detail</b></a>
            </li> -->
            <li class="active">
                <a href="#1" data-toggle="tab"><b>1. Policy Detail</b></a>
            </li>
            <li>
                <a href="#2" data-toggle="tab"><b>2. Nominee</b></a>
            </li>
            <li>
                <a href="#3" data-toggle="tab"><b>3. Family</b></a>
            </li>
            <li>
                <a href="#4" data-toggle="tab"><b>4. BOC</b></a>
            </li>
            <li>
                <a href="#5" data-toggle="tab"><b>5. Other/Info</b></a>
            </li>
            <li>
                <a href="#6" data-toggle="tab" id="nachTab"><b>6. NACH</b></a>
            </li>
            <li>
                <a href="#7" data-toggle="tab"><b>7. Add & Contact</b></a>
            </li>
            <li>
                <a href="#8" data-toggle="tab"><b>8. Medical</b></a>
            </li>
            <li>
                <a href="#9" data-toggle="tab"><b>9. Female Detail</b></a>
            </li>
        </ul>

        <div class="tab-content">
            <!-- @include ('admin.policy.tab_10') -->
            @include ('admin.policy.tab_1')
            @include ('admin.policy.tab_2')
            @include ('admin.policy.tab_3')
            @include ('admin.policy.tab_4')
            @include ('admin.policy.tab_5')
            @include ('admin.policy.tab_6')
            @include ('admin.policy.tab_7')
            @include ('admin.policy.tab_8')
            @include ('admin.policy.tab_9')
        </div>
    </div>
</div>
...
<script type="text/javascript">
    window.polNominee = $.parseJSON('<?php echo $polNominee; ?>');
    window.polBOC = $.parseJSON('<?php echo $polBOC; ?>');
</script>
<script type="text/javascript" src="{{ asset('assets/pages/policy/nomineetab.js') }}"></script>
<script type="text/javascript" src="{{ asset('assets/pages/policy/boctab.js') }}"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/vue/2.4.0/vue.js"></script>

<script type="text/javascript">
    //Vue.component('age-component', require('/resources/components/AgeComponent.vue').default);
    ...
    Vue.component('plan-component', {
        template: '<div><select v-on:change = "plan()" id="PLAN1" name="PLAN" class="form-control" style="width: 100%;"><option value="" selected="selected"></option></select></div>',
        methods: {
            plan: function() {
                if ($("#PLAN").val() != '' && $("#AGE").val() != '' && $("#PLAN").val() > 0 && $("#AGE").val() > 0) {
                    $.ajax({
    ...
    if ($("#PLAN").val() != '' && $("#PLAN").val() > 0) {
        var AGESelected = '<?php echo !empty($policy['AGE']) ? $policy['AGE'] : ''; ?>';
        var MTERMSelected = '<?php echo !empty($policy['MTERM']) ? $policy['MTERM'] : ''; ?>';
        var PTERMSelected = '<?php echo !empty($policy['TERM']) ? $policy['TERM'] : ''; ?>';
        var MODESelected = '<?php echo !empty($policy['MODE']) ? $policy['MODE'] : ''; ?>';
        var FUPSelected = '<?php echo !empty($policy['FUP']) ? $policy['FUP'] : ''; ?>';
        getFupPremDates($("#RDT").val(), MODESelected, MTERMSelected, FUPSelected);
        ...
        if($("#FUP").val() && $("#RDT").val()){
            ...
            var day1 = dateAr[0];
            var month1 = fupdateAr[0];
            var year1 = fupdateAr[1];
            $("#FULLFUP").val(day1 + "/" + month1 + "/" + year1);
        }
        else{
            $("#FULLFUP").val("");  
        }
        ...
        $.ajax({
                ...
                if (!$.isEmptyObject(result)) {
                    ...
                    if (result.getAgeData != '') {
                        for (var i = parseInt(result.getAgeData.agemin); i <= parseInt(result.getAgeData.agemax); i++) {
                            if (AGESelected != '' && AGESelected == i) {
                                $('#AGE').append($("<option selected='selected'></option>").attr("value", i).text(i));
                            } else {
                    ...
                    if (result.getPlanData != '' && parseInt(result.getPlanData.po_term) != 0 && result.getPlanData.po_term != '') {
                        ...
                        $('#MTERM').empty().append('<option selected="selected" value=""></option>');
                        $.each(jsArray, function(r, d) {
                            if (MTERMSelected != '' && MTERMSelected == d) {
                                $('#MTERM').append($("<option selected='selected'></option>").attr("value", d).text(d));
                            } else {
                                $('#MTERM').append($("<option></option>").attr("value", d).text(d));
                            }
                        });
                        ...
                        if (result.getMTermData != '') {
                            for (var i = parseInt(result.getMTermData.mtermmin); i <= parseInt(result.getMTermData.mtermmax); i++) {
                                if (MTERMSelected != '' && MTERMSelected == i) {
                                    $('#MTERM').append($("<option selected='selected'></option>").attr("value", i).text(i));
                                } else {
                                    $('#MTERM').append($("<option></option>").attr("value", i).text(i));
                                }
                            }
                    ...
                    if (result.getPlanData != '' && parseInt(result.getPlanData.pr_term) != 0 && result.getPlanData.pr_term != '') {
                        ...
                        $('#PTERM').empty().append('<option selected="selected" value="' + MTERMSelected + '">' + MTERMSelected + '</option>');
                    } else if (result.getPlanData != '' && parseInt(result.getPlanData.po_pr_same) != 1 && result.getPlanData.sp_po_term != '') {
                        $('#PTERM').empty().append('<option selected="selected" value="' + PTERMSelected + '">' + PTERMSelected + '</option>');
                    } else {
                        ...
                        if (result.getPTermData != '') {
                            for (var i = parseInt(result.getPTermData.ptermmin); i <= parseInt(result.getPTermData.ptermmax); i++) {
                                if (PTERMSelected != '' && PTERMSelected == i) {
                                    $('#PTERM').append($("<option selected='selected'></option>").attr("value", i).text(i));
                                } else {
                    ...
                    if (result.getModeData != '') {
                        $.each(result.getModeData, function(r, d) {
                            ...
                            if (MODESelected != '' && MODESelected == d) {
                                $('#MODE').append($("<option selected='selected'></option>").attr("value", d).text(d));
                            } else {
            ...
            error:function (error) {
                console.log("Get cal detail error 1");
            }
        });
    }
    $("#SAOPTION").hide();
    $(document).on("change", "#PLAN", function() {
        if ($("#PLAN").val() != '' && $("#PLAN").val() > 0) {
            $.ajax({
                ...
                error:function (error) {
                    console.log("Get cal detail error 2");
                }
            });
        } else {
            $('#AGE').empty().append('<option selected="selected" value=""></option>');
    ...
    $(document).on("change", "#MTERM", function() {
        if ($("#PLAN").val() != '' && $("#MTERM").val() != '' && $("#PLAN").val() > 0 && $("#MTERM").val() > 0) {
            ...
            $.ajax({
                ...
                error:function (error) {
                    console.log("Get plan detail error 1");
                }
            });
        }
    });
        ...
        if ($("#PLAN").val() != '' && $("#PLAN").val() > 0) {
            ...
            if ($("#is_auto_prem").prop('checked') == true) {
                $.ajax({
                    ...
                    error:function (error) {
                        console.log("Get premium prentation detail error 1");
                    }
                });
            } else {
    ...
    $(document).on("change", "#bank_sel", function() {
        if ($("#bank_sel").val() != '') {
            $.ajax({
                url: "{{ url('admin/getECSDetail') }}" + "/" + $("#bank_sel").val(),
                method: 'get',
                success: function(result) {
                    if (!$.isEmptyObject(result)) {
                        $("#E_ADD").val(result.AD1 + ' ' + result.AD2 + ' ' + result.AD3);
                        $("#E_BRANCH").val(result.BANKBR);
                        $("#E_MICR").val(result.BANKMICR);
                    }
                },error:function (error) {
                    console.log("Get ecs detail error 1");
                }
            });
        } else {
            $("#E_ADD").val('');
    ...
    $(document).on("click", ".save-policy", function() {
        ...
        if (PROPNO != 0 && PROPDT == '') {
            ...
            alert("Please enter proposal number.");
            return false;
        }
        ...
        //var formPostData = form.serializeArray(); // convert form to array
        var formPostData = new FormData(this.form);
        var finedNomineeList = $('#nominee-section').jsGrid('option', 'data');
        //alert(finedNomineeList);
        if (finedNomineeList.length > 0) {
            var totalShare = 0;
            $.each(finedNomineeList, function(k, v) {
                $.each(v, function(r, d) {
                    formPostData.append("NOMINEE[" + k + "][" + r + "]", d);
                    //formPostData.push({name : "NOMINEE["+k+"]["+r+"]", value : d});
                    if (r == 'SHARE') {
                        totalShare += d;
                    }
                });
            ...
            if (totalShare > 100) {
                alert("Please change nominee share value. You can't add more than 100.");
                return false;
            }
        ...
        //return false;
        var finedBOCList = $('#branch-office-section').jsGrid('option', 'data');

        if (finedBOCList.length > 0) {
            $.each(finedBOCList, function(k, v) {
                $.each(v, function(r, d) {
                    formPostData.append("BOC[" + k + "][" + r + "]", d);
                    //formPostData.push({name : "BOC["+k+"]["+r+"]", value : d});
                });
            });
        }
    ...
    $(document).on("change", "#NACH", function() {
        if ($("#NACH").val() == 'No') {
            $("#nachTab").hide();
            //$(".nachTabDeails").hide();
        } else {
